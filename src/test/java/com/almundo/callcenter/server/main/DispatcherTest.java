...
public class DispatcherTest {
	...
	*/
	...
	private int amountFinishCalls = 0;
    ...
    public void testDispatchTenCallsToEmployees() throws InterruptedException {
        ...
        while(true){
	        ...
	        if (Long.valueOf(dispatcher.getExecutorServiceCall().getCompletedTaskCount()).intValue() == Constants.CALL_AMOUNT){
	        	...
	        	dispatcher.getQueryCallsDTO().forEach(c -> {
	        		if(c.getCallStatus().equals(CallStatus.FINISHED)){
	        			this.amountFinishCalls++;
	        			
	        		}
	        	});
	        	if (this.amountFinishCalls == Constants.CALL_AMOUNT){
